...
1. Initialize an empty vector ans to store the left view nodes.
2. If the root is null, return the empty vector.
3. Create a queue q and push the root node into it.
4. Run a loop until the queue is not empty.
Get the size of the current level of the tree.
Push the data of the front node of the queue (leftmost node in the current level) into the ans vector.
Enqueue the left and right children of all nodes in the current level (if they exist) into the queue.
