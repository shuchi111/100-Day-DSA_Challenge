...
Initialize a 2D vector dp of size n x n to store the minimum cost of multiplying matrices and the corresponding split position.
Set dp[i][i] and dp[i][i + 1] to {0, -1} for all valid i.
For each subsequence length L from 2 to n - 1:
Iterate over all possible starting indices i.
Compute the cost of multiplying matrices for the current subsequence using the formula p[i] * p[j] * p[k] + dp[i][k].first + dp[k][j].first.
If the current cost is less than the existing cost, update dp[i][j].
Build the matrix multiplication sequence using a recursive function helper:
Initialize a vector res to store the result.
Use the split positions stored in dp to construct the sequence.
