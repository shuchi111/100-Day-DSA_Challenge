...
/*
Asked in company - Amazon, DE-shaw. Hike, paytm , SAPlab, snapdeal

problem - Given a linked list of N nodes. The task is to check if the linked list has a loop. Linked list can contain self loop.
...
Check if slow is equal to fast:
If true, there is a loop; return true.
If false, continue the loop.
3. If the loop exits, there is no loop in the linked list; return false.
...
class Solution
{
    ...
    {
        // your code here 
        // just used approach of middle element in li ked list
        Node* slow = head;
        Node* fast = head;
