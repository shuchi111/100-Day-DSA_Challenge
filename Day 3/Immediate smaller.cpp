...
If arr[i] > arr[i+1], update arr[i] with arr[i+1].
Otherwise, update arr[i] with -1.
2. Set arr[n-1] to -1 (since there is no element to its right).
3. The array arr is now updated as required.
